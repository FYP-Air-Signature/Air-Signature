...
class ModelVerifier:
    ...
    def verify(self, user, new_img_name="new_sign.png"):
        results = []
        verification_images_dir = os.path.join('application_data', user, 'verification_images')
        ...
        for image in verification_images:
            input_img = self.preprocess(os.path.join('application_data', user, 'new_sign', new_img_name))
